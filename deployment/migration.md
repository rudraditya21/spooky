...
**Envoy's xDS dynamic control plane** does not apply to Spooky. Spooky uses a static YAML config and requires a process restart (or SIGHUP if supported by your version) to pick up config changes. If your current setup relies on Envoy's ADS/EDS for dynamic backend discovery (e.g., service discovery via Consul or a service mesh), you cannot replicate that behavior with Spooky today. Alternatives: use a configuration management tool (Ansible, Puppet, Terraform) to render Spooky's config and restart on change, or continue using Envoy for the service mesh interior and put Spooky only at the edge.
